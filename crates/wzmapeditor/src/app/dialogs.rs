...
/// Modal shown after a user-initiated `.wz` open or drag-drop fails.
///
/// The message is keyed off the archive classification so wrong-file
/// scenarios (script maps, mods, plain zips) get a specific explanation
/// instead of the raw loader error.
pub(super) fn show_load_error_dialog(ctx: &egui::Context, app: &mut EditorApp) {
    let mut open = app.load_error_dialog.open;
    let mut dismiss = false;
    let title = app.load_error_dialog.title.clone();
    egui::Window::new(title)
        ...
        .show(ctx, |ui| {
            ...
            ui.label(&app.load_error_dialog.message);
            if !app.load_error_dialog.details.is_empty() {
                ui.add_space(4.0);
                ui.label(
                    egui::RichText::new(&app.load_error_dialog.details)
                        .monospace()
                        .weak(),
                );
            }
    ...
    if dismiss || !open {
        app.load_error_dialog.open = false;
    }
}

/// Title and human-readable explanation for a failed `.wz` load.
///
/// The classification is best-effort: when the loader has already returned
/// an error we re-open the archive just to peek at its entry list, and the
/// `kind` here narrows the message. Unknown/unclassifiable archives fall
/// back to the loader error text verbatim.
pub(crate) fn wz_load_error_copy(kind: &WzArchiveKind, raw_error: &str) -> (String, String) {
    match kind {
        WzArchiveKind::ScriptMap => (
            "Script maps can't be edited".to_string(),
            "The terrain is generated by game.js at runtime, so there's no \
             static map for the editor to load. Try a regular map instead."
                .to_string(),
        ),
        WzArchiveKind::Empty => (
            "Empty archive".to_string(),
            "The .wz file has no entries.".to_string(),
        ),
        WzArchiveKind::NotAnArchive => (
            "Not a valid .wz archive".to_string(),
            "The file isn't a readable zip.".to_string(),
        ),
        WzArchiveKind::Unknown => (
            "Not a Warzone map".to_string(),
            "No game.map found in the archive.".to_string(),
        ),
        WzArchiveKind::Map | WzArchiveKind::MultiMapPack { .. } => {
            ("Could not open map".to_string(), raw_error.to_string())
        }
    }
}
